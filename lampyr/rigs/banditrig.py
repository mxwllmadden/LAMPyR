...
from lampyr.rigs.interfaces import SerialInterface_ArduinoTRV
...
class Wheel(Component):
    ...
    def movement_since(self, time):
        condition = lambda report : report['unix_time'] > time
        values = self.serial.data.scanback_values('R', 'report_value', condition)
...
class Lick(Component):
    ...
    def since(self, time):
        ...
        """
        condition = lambda report : report['unix_time'] > time
        lickdat = self.serial.data.scanback_values('L', 'report_value', condition)
        lcount = 0
